...
const loremWrapper = document.querySelector('.lorem-text');

loremForm.addEventListener('submit', (e) => {
  e.preventDefault();
  const paraNum = Number(inputNumber.value);
  let paras = [];
  
  for (let i = 0; i < paraNum; i++) {
    const randNum = Math.floor(Math.random() * text.length);
    paras.push(text[randNum]);
  }
  
  loremWrapper.innerHTML = paras.map(para => `<p class="result">${para}</p>`).join('');
});
